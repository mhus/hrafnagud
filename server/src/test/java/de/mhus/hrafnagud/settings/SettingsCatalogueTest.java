 ...
 * on: every key names a layer that owns it and is one an operator can find in
 * {@code application.yml}, and every one of them says what it does. Both are
 * the kind of thing that decays one hurried entry at a time.
 */
class SettingsCatalogueTest {
    ...
    /**
     * One of the two layer roots, never a third and never bare: a key says
     * whether the value belongs to collecting and storing or to thinking, and
     * that is the whole reason the roots were split.
     */
    @Test
    void every_declared_key_names_the_layer_that_owns_it() {
        List<Setting<?>> declared = fixture.store().declared();

        assertThat(declared).isNotEmpty();
        assertThat(declared).allSatisfy(setting ->
                assertThat(setting.key()).matches("^(munin|hugin)\\..+"));
    }
