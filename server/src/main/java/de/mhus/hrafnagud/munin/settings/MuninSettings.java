 ...
 *
 * <p>This is the list. A key exists because it is written here, its type and
 * its default come from here, and the sentence an operator reads next to it in
 * the console is the one written here — so there is no second place where a
 * setting could be half-defined.
 *
 * <p>The sections mirror {@link MuninProperties} field for field, and the keys
...
public class MuninSettings {
    ...
    public MuninSettings(SettingsService store, MuninProperties defaults) {
        ...
        MuninProperties.Translation translationDefaults = defaults.getTranslation();
        this.translation = new Translation(
                store.bool("munin.translation.enabled", translationDefaults::isEnabled,
                        "Whether the worker drains the translation backlog. Off leaves the "
                                + "queue filling, which is the reason this is separate from "
                                + "the pivot language."),
                store.text("munin.translation.pivotLanguage",
                ...
                store.languages("munin.translation.readableLanguages",
                ...
                store.bool("munin.translation.translateSummary",
                ...
                store.integer("munin.translation.batchSize",
                        translationDefaults::getBatchSize,
                        "Articles claimed per round."),
                store.integer("munin.translation.maxAttempts",
                        translationDefaults::getMaxAttempts,
                        "Attempts before an article is dropped from the backlog."),
                store.duration("munin.translation.retryDelay",
                        translationDefaults::getRetryDelay,
                        "Delay before the first retry; doubles per attempt."),
                store.duration("munin.translation.claimLease",
                        translationDefaults::getClaimLease,
                        "Lease held on a claimed article."),
                store.integer("munin.translation.maxSourceChars",
        ...
        MuninProperties.Category categoryDefaults = defaults.getCategory();
        this.category = new Category(
                store.bool("munin.category.enabled", categoryDefaults::isEnabled,
                        "Whether unresolved category mappings are sent to a brain. The "
                                + "table-matching stage runs either way and costs nothing."),
                store.integer("munin.category.batchSize", categoryDefaults::getBatchSize,
                        "Mappings resolved per round."),
                store.integer("munin.category.maxAttempts", categoryDefaults::getMaxAttempts,
                        "Attempts before a mapping is left FAILED for a person to settle."),
                store.duration("munin.category.retryDelay", categoryDefaults::getRetryDelay,
                        "Delay before the first retry; doubles per attempt."),
                store.duration("munin.category.claimLease", categoryDefaults::getClaimLease,
                        "Lease held on a claimed mapping."),
                store.fraction("munin.category.acceptConfidence",
                        categoryDefaults::getAcceptConfidence,
